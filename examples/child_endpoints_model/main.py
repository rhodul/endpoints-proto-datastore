...
# MyParentInsert to allow API users to create.
...
class ChildEndpointsModel(EndpointsModel):
  ...
  def SetKey(self):
    # If _parent ID is present, let's set the key, so we know who the parent is.
    if self._parent is not None:
      ...
      if self._id is not None:
        # We have an _id? That means that this is update/retrieve of existing child.
...
class MyParent(EndpointsModel):
  ...
  # custom ProtoRPC message schema. We added the built in ID property
...
class MyApi(remote.Service):
  ...
  # "parent" in the request path by setting path='mymodel/{parent}'.
  # Unlike in keys_with_ancestors/main.py, we want NDS to generate id
  # and so we do not include it in request body.
  ...
  def MyModelUpdate(self, my_model):
    if not my_model.from_datastore:
      raise endpoints.BadRequestException(
          'MyModel %s with parent %s does not exists.' %
